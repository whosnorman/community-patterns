...
import type { GoogleDocsDocument, GoogleComment } from "./google-docs-markdown.ts";
export type { GoogleDocsDocument, GoogleComment };
...
export class GoogleDocsClient {
  ...
  private async request(
    ...
    if (status === 401) {
      ...
      debugLog(this.debugMode, `Rate limited, incrementing delay to ${this.delay}ms`);
      await sleep(this.delay);
    }
  ...
  async listComments(
    ...
    do {
      ...
      debugLog(
        this.debugMode,
        `Fetched ${comments.length} comments (page token: ${pageToken || "none"})`
      );
