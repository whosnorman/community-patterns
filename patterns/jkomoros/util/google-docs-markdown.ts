...
export async function downloadImageAsBase64(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[downloadImageAsBase64] Failed to download image: ${response.status}`);
      return null;
    }
...
function formatCommentAsBlockquote(comment: GoogleComment): string {
  ...
  // Main comment
  lines.push(`> **${comment.author.displayName}** (${formatDate(comment.createdTime)}):`);
  ...
  if (comment.replies && comment.replies.length > 0) {
    for (const reply of comment.replies) {
      ...
      lines.push(`>`);
      lines.push(`> > **${reply.author.displayName}** (${formatDate(reply.createdTime)}):`);
    ...
    if (element.paragraph) {
      ...
      for (const elem of paragraphElements) {
        if (elem.textRun) {
          ...
          if (embeddedObj) {
            const imageUrl =
              embeddedObj.imageProperties?.contentUri ||
              embeddedObj.imageProperties?.sourceUri;
            const altText = embeddedObj.title || embeddedObj.description || "Image";
      ...
      if (para.bullet) {
        ...
        const marker = getListMarker(doc, listId, nestingLevel, currentListState);
