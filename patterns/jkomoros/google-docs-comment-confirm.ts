...
class GoogleDocsClient {
  ...
  async createReply(
    ...
    if (!res.ok) {
      ...
      if (res.status === 403) {
        throw new Error("Access denied. You may not have permission to comment on this document.");
      }
...
>(async (_, {
  ...
  try {
    ...
    if (resolve) {
      ...
      comments.set(currentComments.filter((c) => c.id !== pendingAction.commentId));
    }
    ...
    const errorMessage = e instanceof Error ? e.message : "Failed to post reply";
