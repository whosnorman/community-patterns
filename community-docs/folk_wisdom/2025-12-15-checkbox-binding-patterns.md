...
- 2025-12-15 - **CELL.MAP() PROPERTY ACCESS**: Inside Cell.map(), ALL object properties are auto-wrapped as Cells. This means: (1) Cannot call `.get()` on properties - causes "space is required" error, (2) Cannot compare values directly (`s.status === "foo"` compares Cell to string - always false), (3) Framework auto-unwraps for JSX text content (`{s.name}` works), (4) `$checked` binding handles cells directly. **Solution**: Pre-compute ALL display-related values (colors, emojis, conditional classes) at population time and store as string properties on the objects. (extracurricular-color-fix)
